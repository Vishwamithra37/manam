...
def makecomment():   
    ...
    try:
        ...
        assert type(data["user"]["solution"])==str
        assert type(data["user"]["m_question"])==str
    except:    
        return {"error":"Wrong type sent"}
    try:
             ######################Veriying the token###########################################
             dab=db["Tokens"];dac=dab[data["user"]["state"]];dad=dac[data["auth"]["token"]]              
             fire=dad.find_one({},{"_id":0,"user.nation":1,"user.email":1,"user.state":1,"user.election circles":1})                                                                                         
             if fire==None:
                    return {"Error":"User Does not exist"}
           ######################Veriying the token###########################################
             else:   
                dab1=db["Feed"];dac1=dab1[data["user"]["state"]];dad1=dac1[data["auth"]["token"]] 
